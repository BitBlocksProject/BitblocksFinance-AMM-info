...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://exchange.bitblocks.finance/#/` +
      (remove ? `remove` : `add`) +
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://exchange.bitblocks.finance/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://exchange.bitblocks.finance/#/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export function getTimestampsForChanges() {
  const utcCurrentTime =dayjs.unix(Date.now() / 1000 | 0)
  const t1 = utcCurrentTime.subtract(1, 'day').startOf('minute').unix()
  const t2 = utcCurrentTime.subtract(2, 'day').startOf('minute').unix()
  let tWeek = utcCurrentTime.subtract(1, 'week').startOf('minute').unix()
...
export async function getBlockFromTimestamp(timestamp) {
  ...
  if(timestamp < 1651101307){
    timestamp = 1651106307
  }
  
  let result = await blockClient.query({
    query: GET_BLOCK,
    variables: {
      timestampFrom: timestamp - 5000,
...
export const formatTime = (unix) => {
  const now = dayjs.unix(Date.now() / 1000 | 0)
