...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "bitblocksproject/refactorblocks") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GET_BLOCK = gql`
  query blocks($timestampFrom: BigInt, $timestampTo: BigInt) {
    blocks(
      first: 1
      orderBy: timestamp
      orderDirection: asc
...
export const PAIR_DAY_DATA_BULK = (pairs, startTimestamp) => {
  ...
  const queryString = `
    query days {
      pairDayDatas(first: 1000, orderBy: date, orderDirection: asc, where: { pairAddress_in: ${pairsString} }) {
        id
        pairAddress
        date
        dailyVolumeToken0
...
export const GLOBAL_CHART = gql`
  query pancakeDayDatas($startTime: Int!, $skip: Int!) {
    pancakeDayDatas(first: 1000, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
